...
def process_file(row, data_dir):
    ...
    if os.path.exists(audio_file_path):
        ...
        if transcribed_text:
            ...
            return row[audio_column], transcribed_text
        else:
            logging.warning(f"Failed to transcribe {audio_file_path}")
            return audio_file_path, "Failed to transcribe"
    else:
        logging.warning(f"Audio file not found: {audio_file_path}")
        return audio_file_path, "Not found audio file"
...
if __name__ == "__main__":
    ...
    for index, row in tqdm(df.iterrows(), total=len(df), desc="Processing files"):
        path, transcribed_text = process_file(row, args.data_dir)
        result = {
            "index": index,
            "audio_file": path,
            "transcribed_text": transcribed_text,
            "original_transcription": row["sentence"]  # Add original transcription/source sentence
        }
