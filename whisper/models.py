...
sys.path.append('/data/daisysxm76/speechtospeech/ltu-main/src/ltu_as/whisper/whisper/')

from model import ResidualAttentionBlock, Linear
...
class TLTR(nn.Module):
    def __init__(self, label_dim=527, n_layer=33, rep_dim=1280, mode='lw_8_8_tr'):
        super().__init__()
        self.mode = mode
        ...
        logging.debug(f"Initializing TLTR with mode: {mode}")
        ...
        if mode == 'mean_mlp' or mode == 'last_mlp':
        ...
        elif 'mean_tr' in mode or 'last_tr' in mode:
        ...
        elif 'lw_tr' in mode or 'lw_8_8_tr' in mode:
            self.num_tatt_head = int(mode.split('_')[-3])
            self.num_latt_head = int(mode.split('_')[-2])
            ...
            logging.error(f"Unknown mode '{mode}' for TLTR model")
            raise ValueError(f"Unknown mode '{mode}' for TLTR model")

    def forward(self, audio_rep):
        logging.debug(f"Forward pass with audio_rep shape: {audio_rep.shape}")

        if self.mode == 'mean_mlp':
            audio_rep = torch.mean(audio_rep, dim=1)
            audio_rep = torch.mean(audio_rep, dim=1)
            audio_rep = self.mlp_layer(audio_rep)
            logging.debug(f"Output shape (mean_mlp): {audio_rep.shape}")
            return audio_rep
            ...
            audio_rep = torch.mean(audio_rep, dim=1)
            audio_rep = self.mlp_layer(audio_rep)
            logging.debug(f"Output shape (last_mlp): {audio_rep.shape}")
            return audio_rep

        elif self.mode == 'wa_mlp':
            audio_rep = torch.mean(audio_rep, dim=2)
            audio_rep = torch.permute(audio_rep, (0, 2, 1))
            audio_rep = (audio_rep @ self.layer_weight) / self.layer_weight.sum()
            audio_rep = self.mlp_layer(audio_rep)
            logging.debug(f"Output shape (wa_mlp): {audio_rep.shape}")
            return audio_rep
            ...
            audio_rep = torch.mean(audio_rep, dim=1)
            audio_rep = self.mlp_layer(audio_rep)
            logging.debug(f"Output shape (mean_tr): {audio_rep.shape}")
            return audio_rep
            ...
            audio_rep = torch.mean(audio_rep, dim=1)
            audio_rep = self.mlp_layer(audio_rep)
            logging.debug(f"Output shape (last_tr): {audio_rep.shape}")
            return audio_rep

        elif 'wa_tr' in self.mode:
            audio_rep = torch.permute(audio_rep, (0, 2, 3, 1))
            audio_rep = (audio_rep @ self.layer_weight) / self.layer_weight.sum()
            audio_rep = self.time_tr(audio_rep)
            audio_rep = torch.mean(audio_rep, dim=1)
            audio_rep = self.mlp_layer(audio_rep)
            logging.debug(f"Output shape (wa_tr): {audio_rep.shape}")
            return audio_rep
            ...
            audio_rep = torch.mean(audio_rep, dim=1)
            audio_rep = self.mlp_layer(audio_rep)
            logging.debug(f"Output shape (wa_down_tr): {audio_rep.shape}")
            return audio_rep

        elif 'lw_tr' in self.mode or 'lw_8_8_tr' in self.mode:
            # Reshape the tensor to [-1, 1500, 1280]
            # try:
            #     audio_rep = audio_rep.reshape(-1, 1500, 1280)
            #     logging.debug(f"Reshaped audio_rep shape: {audio_rep.shape}")
            # except RuntimeError as e:
            #     logging.error(f"Error during reshaping: {e}")
            #     return None

            # # Proceed with further processing
            # audio_rep = self.time_tr(audio_rep)
            # audio_rep = torch.mean(audio_rep, dim=1)
            # audio_rep = audio_rep.reshape(-1, self.n_layer, audio_rep.shape[-1])
            # audio_rep = self.layer_tr(audio_rep)
            # audio_rep = torch.mean(audio_rep, dim=1)
            # audio_rep = self.mlp_layer(audio_rep)

            # logging.debug(f"Output shape (lw_tr): {audio_rep.shape}")
            # return audio_rep
            ...
            B = audio_rep.shape[0]
            print(audio_rep.shape)
            audio_rep = audio_rep.reshape(B * self.n_layer, audio_rep.shape[2], audio_rep.shape[3])
            audio_rep = self.time_tr(audio_rep)
            audio_rep = torch.mean(audio_rep, dim=1)
            audio_rep = audio_rep.reshape(B, self.n_layer, audio_rep.shape[1])
            audio_rep = self.layer_tr(audio_rep)
            audio_rep = torch.mean(audio_rep, dim=1)
            audio_rep = self.mlp_layer(audio_rep)
            logging.debug(f"Output shape (lw_tr): {audio_rep.shape}")
            ...
            audio_rep = torch.mean(audio_rep, dim=1)
            audio_rep = self.mlp_layer(audio_rep)
            logging.debug(f"Output shape (lw_down_tr): {audio_rep.shape}")
            return audio_rep

        else:
            logging.error("No valid mode found for TLTR forward pass")
            return None
...
# Add the directory containing 'model.py' to the sys.path
sys.path.append('/data/daisysxm76/speechtospeech/ltu-main/src')

from ltu_as.whisper.whisper.model import ResidualAttentionBlock, Linear
